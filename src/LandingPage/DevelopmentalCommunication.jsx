...
export const DevelopmentalCommunication = ({ articles }) => {
  // Filter the articles array to get only articles with the "Sports" category
  const developmentalCommunicationsArticles = articlesData.filter((article) => article.category === 'Developmental Communication');

  return (
    <Fragment>
      <NavigationBar/>
      <div className="bg-green text-white pt-40 pb-20 text-center">
      <Typography className="text-6xl font-medium">Developmental Communication</Typography>
    </div>
      <div className="px-52 py-24">
      <h2 className="text-2xl font-bold mb-4">Recent Articles</h2>
      <ul>
        {developmentalCommunicationsArticles.map((article) => (
          <li key={article.id}>
